...
function Wipe({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden rounded-[28px] h-full">
      <img src={src} alt={alt} loading="lazy" decoding="async" className="object-cover w-full h-full" />
      <motion.div
        className="absolute inset-y-0 left-0 w-px bg-[var(--gold)]"
        initial={{ scaleX: 0, x: 0 }}
        animate={inView ? { x: ["0%", "100%"] } : {}}
        style={{ originX: 0, width: "100%", height: "100%", background: "linear-gradient(90deg, transparent 0%, transparent 49.9%, var(--gold) 50%, transparent 50.1%, transparent 100%)" }}
...
function ZoomIn({ src, alt }: { src: string; alt: string }) {
  return (
    <div className="relative overflow-hidden rounded-[28px] h-full group">
      <img src={src} alt={alt} loading="lazy" decoding="async" className="object-cover w-full h-full transition-transform duration-[2500ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.08]" />
    </div>
  );
}
...
function ZoomOut({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden rounded-[28px] h-full">
      <motion.img src={src} alt={alt} loading="lazy" decoding="async" style={{ scale }} className="object-cover w-full h-full" />
    </div>
  );
}
    ...
    >
      ...
      >
        <div className="absolute inset-0" style={{ backfaceVisibility: "hidden" }}>
          <img src={src} alt={alt} loading="lazy" decoding="async" className="object-cover w-full h-full" />
          <span className="absolute top-3 right-3 text-[10px] uppercase tracking-[0.2em] px-2 py-1 rounded-full bg-black/50 text-white">Click to flip</span>
        </div>
        <div className="absolute inset-0 flex items-center justify-center bg-[var(--charcoal)] text-[var(--sand)]"
          style={{ transform: "rotateY(-90deg) translateZ(1px)", backfaceVisibility: "hidden" }}>
          <img src={back} alt="Evening view" className="object-cover w-full h-full absolute inset-0 opacity-70" />
...
export function Gallery() {
  ...
  const tiles = [
    { key: "p7",  el: <Wipe src={p7.src} alt={p7.alt} />,       cls: "md:row-span-2 h-[460px] md:h-full", idx: 6 },
    { key: "p8",  el: <ZoomIn src={p8.src} alt={p8.alt} />,     cls: "h-[300px]", idx: 7 },
    { key: "p9",  el: <ZoomOut src={p9.src} alt={p9.alt} />,    cls: "h-[300px]", idx: 8 },
    { key: "p10", el: <BlurDissolve src={p10.src} alt={p10.alt} />, cls: "h-[460px] lg:row-span-2", idx: 9 },
    { key: "p11", el: <CubeFlip src={p11.src} alt={p11.alt} back={p2.src} />, cls: "h-[300px]", idx: 10 },
    { key: "p12", el: <PageTurn src={p12.src} alt={p12.alt} />, cls: "h-[300px]", idx: 11 },
  ];
    ...
    <section id="gallery" className="py-24 md:py-32" style={{ background: "var(--sand-soft)" }}>
      <div className="mx-auto max-w-[1200px] px-6 lg:px-12">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h2 className="h1-lux">Six frames<br/>from the penthouse</h2>
        </div>
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 auto-rows-[300px] gap-7">
          {tiles.map((t) => (
            <div
              key={t.key}
              className={`relative cursor-zoom-in ${t.cls}`}
              onClick={() => ui.openLightbox(t.idx)}
            >
              {/* Photo {idx+1}/20 — Gallery tile */}
              {t.el}
            </div>
          ))}
        </div>
